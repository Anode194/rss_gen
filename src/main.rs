...
fn main() {
    ...
    let header_garbage = match strip_till_title(contents.as_mut_str()) { // should return tuple of .0=header_garbage .1 rest of the contents of the file.
        Ok(i) => i,
            
        Err(e) => 
            panic!("failed to parse header garbage \n{:?}",e),
    };

    let mut header = match parse_title(header_garbage.0) {
        Ok(i) => i,
        Err(e) =>
            panic!("parse blogpost title failed \n{:?}",e),
    };
    ...
    let mut body = match strip_suffix(header.0) {
        Ok(i) => i.1,
        Err(e) =>
            panic!("failed to strip prefix from body. \n{:?}",e),
    };
